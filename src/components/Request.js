...
export default class RequestFunds extends React.Component {
  ...
  render () {
    const { canRequest, amount, requested } = this.state
    const { address, buttonStyle, changeAlert, token, networkId } = this.props
    if (requested) {
      const invoice = {
        network: networkId,
        publicKey: address,
          ...
          <div className='content row'>
            <div className='form-group w-100'>
              ...
              <div className='input-group'>
                ...
                <input
                  type='number' className='form-control' placeholder='0.00' value={this.state.amount}
